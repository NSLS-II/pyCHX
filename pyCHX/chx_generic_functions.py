...
def trans_data_to_pd(data, label=None,dtype='array'):
    ...
    index =  arange( N )
    if label == None:label=['data%s'%i for i in range(M)]
    #print label
...
def ring_edges(inner_radius, width, spacing=0, num_rings=None):
    ...
    try:
        iter(width)
        width_is_list=True
    except: width_is_list=False
    try:
        iter(spacing)
        spacing_is_list=True
    except: spacing_is_list=False
